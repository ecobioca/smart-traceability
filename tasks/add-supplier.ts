...
task("add-supplier", "Add new supplier")
  ...
  .addParam("holder", "Holder address, can be a contract", undefined, types.string)
  .setAction(async (taskArgs, hre) => {
    const contract = await hre.ethers.getContractAt("Traceability", taskArgs.contract);

    const [owner] = await hre.ethers.getSigners();

    const tx = await contract.addSupplier(taskArgs.metadata, [taskArgs.manager], taskArgs.holder, 0, { from: owner.address });
    const receipt = await tx.wait();

    const supplierId = receipt.events.filter((e: any) => e.event === 'NewSupplier')[0].args.id.toString();
